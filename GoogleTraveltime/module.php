...
class GoogleDistanceTraveltime extends IPSBaseModule
{
    protected $config = array('gdt_api_key', 'gdt_origin', 'gdt_destination', 'gdt_interval');

    public function Create()
    {
        parent::Create();

        $this->RegisterPropertyString('gdt_api_key', '');
        $this->RegisterPropertyString('gdt_origin', '');
        $this->RegisterPropertyString('gdt_destination', '');
        $this->RegisterPropertyInteger('gdt_interval', 5);

        if (!IPS_VariableProfileExists('LOCI.Minutes')) {
            IPS_CreateVariableProfile('LOCI.Minutes', 1);
        }

        $this->RegisterTimer('update', $this->ReadPropertyInteger('gdt_interval'), 'LOCIGDT_Update($_IPS[\'TARGET\']);');
    }

    protected function OnConfigValid()
    {
        $this->SetTimerInterval('update', $this->ReadPropertyInteger('gdt_interval') * 1000 * 60);
    ...
    {
        $api_key = $this->ReadPropertyString('gdt_api_key');
        $origin = urlencode($this->ReadPropertyString('gdt_origin'));
        $destination = urlencode($this->ReadPropertyString('gdt_destination'));
